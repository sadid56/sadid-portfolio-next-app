      ...
      <div className="App" ref={componentRef}>
        <div ref={sliderRef} className="container">
          {projects?.map((project) => (
            ...
            <ShineBorder color={["#03e9f4", "#FE8FB5", "#094f52c8"]} key={project?.id} className="panel flex w-full my-auto mx-5 snap-center  md:w-[80%] h-[70%] md:h-[60%]">
              ...
              <ProjectCardSpotlight className="flex flex-col md:flex-row gap-4 md:gap-7 rounded-sm">
                ...
                <div className="md:w-[60%] space-y-4 z-20">
                  <h3 className=" text-xl md:text-3xl font-semibold text-slate-300 bg-[#108a91] py-1">{project?.project_name}</h3>
                  <p className="text-sm md:text-[16px] text-slate-300 leading-6">{project?.description}</p>

                  {/* technologies */}
                  <h5 className="text-[12px] md:text-sm text-slate-300 font-medium">Technologies: <span className="text-slate-400">{project?.technology?.join(", ")}</span></h5>
